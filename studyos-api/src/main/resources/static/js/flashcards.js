...
/* ══════════════════════════════════════════════
   ...
   Suporta: `inline code`, ```blocos```, **bold**, _italic_, listas
══════════════════════════════════════════════ */
function parseContent(raw) {
  ...
  // quebras de linha → parágrafos simples
  raw = raw.split('\n').map(line => {
...
async function load() {
  try {
    ...
    console.error(err);
    renderGrid([]);
    if (typeof Toast !== 'undefined') Toast.error('Erro ao carregar flashcards.');
  }
}
...
function renderSummary() {
  ...
  // streak: dias consecutivos com pelo menos 1 revisão (simplificado — mostra repetitions máx)
...
function renderSubjectChips() {
  ...
  const seen = new Map();
  allCards.forEach(c => {
    if (!seen.has(c.subjectId)) seen.set(c.subjectId, { id: c.subjectId, name: c.subjectName, color: c.subjectColor, icon: c.subjectIcon });
  });
...
function applyFilter() {
  ...
  if (activeFilter === 'all') {
    ...
    filtered = allCards.filter(c => !c.nextReviewAt || new Date(c.nextReviewAt) <= now);
  } else if (activeFilter === 'new') {
    filtered = allCards.filter(c => (c.repetitions ?? 0) === 0);
  } else if (activeFilter === 'mastered') {
    filtered = allCards.filter(c => (c.repetitions ?? 0) >= 3 && (c.easeFactor ?? 2.5) >= 2.5);
        ...
        <div class="fc-card-body">
          ...
          <div class="fc-card-footer">
            ...
            <div class="fc-card-actions">
              <button class="fc-icon-btn" title="Estudar este card" onclick="startStudy('single', ${c.id})">▶</button>
...
async function deleteCard(id) {
  ...
  try {
    ...
    renderSubjectChips();
    applyFilter();
    if (typeof Toast !== 'undefined') Toast.success('Flashcard apagado.');
  } catch (err) {
    if (typeof Toast !== 'undefined') Toast.error('Erro ao apagar.');
  }
}
...
async function createCard() {
  ...
  try {
    ...
    applyFilter();
    closeCreate();
    if (typeof Toast !== 'undefined') Toast.success('Flashcard criado!');
  } catch (err) {
...
function startStudy(mode, singleId) {
  ...
  if (!studyQueue.length) {
    if (typeof Toast !== 'undefined') Toast.error('Nenhum card para estudar.');
    return;
  }
...
function showStudyCard() {
  ...
  // syntax highlight nos blocos recém inseridos
